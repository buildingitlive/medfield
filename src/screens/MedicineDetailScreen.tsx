...
import { ArrowLeft, ShieldCheck, Share2, CheckCircle2, ShoppingCart } from 'lucide-react';
...
export const MedicineDetailScreen: React.FC<MedicineDetailScreenProps> = ({
  ...
  const mrpPrice = (product.price + 25.0).toFixed(2);
    ...
    <main className="min-h-screen pb-32 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-1.5 text-sm font-semibold text-primary-container mb-1">
          <span>By {product.grower.name}</span>
        ...
        <p className="text-xs text-on-surface-variant">
          {product.genericName} • Formulation Batch #{product.batchNumber}
        </p>
      </div>

      {/* Composition & Pack Size Card (matching Reference Design) */}
